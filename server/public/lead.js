...
function resetAll() {
  ...
  el.nameInput.value = '';
  api('POST', '/api/reset-all').then(data => {
    applyState(data);
    pollStats();
    if (data.sessionsMilestone) celebrateMilestone(data.sessionsMilestone, 'session');
  });
}
...
function celebrateSupport(name, count, isOwnTip) {
  ...
  shakeTimer = setTimeout(() => el.app.classList.remove('support-shake'), 7700);
  ...
  // itself for the duration: it's a sibling of .app, not a descendant, so
  // it never shook, but the dark backdrop (a higher z-index) was covering
  // it in its usual corner spot. Inside the overlay it sits calmly below
  // the thank-you text and stays on top of the backdrop.
...
el.supportBtn.addEventListener('click', () => {
  ...
  fetch('/api/tip', {
    ...
    body: JSON.stringify({ name: t('leadTipperLabel') })
  }).catch(() => {});
